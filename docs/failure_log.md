...
## [2026-05-03] — isTrusted=false Submit Events Arkose MatchKey Deteksiyonuna Takılıyor (GHOST SUBMIT)

**Hata:** `injectAnswerOnTarget()` submit için `element.click()`, `form.submit()`, `KeyboardEvent.dispatchEvent()` kullanıyordu. Bunların HEPSİ `isTrusted=false` JavaScript-tabanlı event üretiyor. Arkose Labs MatchKey sistemi submit butonu event'lerinde `isTrusted=true` kontrolü yapıyor. Bu yüzden submit sunucuya ulaşsa bile ("reached_server=true") Arkose cevabı yok sayıyor ve DOM'da "wrong" veya "complete" text'i oluşmuyor. Sonuç: pollPostSubmitVerdict hep `.unknown` dönüyor, pipeline ilerlemiyor.

**Kök sebep:** JS `element.click()` → `isTrusted=false`. `Event.isTrusted` özelliği yalnızca gerçek kullanıcı etkileşiminde `true` olur (browser input layer → OS event → DOM event). Programatik `click()` her zaman `isTrusted=false` üretir. CDP `Input.dispatchMouseEvent` ise OS-level input simüle eder ve `isTrusted=true` event üretir. Arkose Labs MatchKey, bot deteksiyonunun bir parçası olarak submit event `isTrusted` değerini kontrol eder.

**Kaynak:**
- CDP spec — `Input.dispatchMouseEvent` generates input at browser compositor/input layer
- MDN Event.isTrusted — "true when event was generated by user action, false when script created/dispatched"
- Medium/FunCaptcha analizi (2025-10-16): "Arkose collects mouse-movement patterns, timing, environmental signals"
- ChromeDevTools Protocol Input domain — dispatchMouseEvent produces trusted events

**Düzeltme:**
1. `browser_bridge.zig`: `CdpClient.dispatchMouseClickAt(x, y)` eklendi — CDP `Input.dispatchMouseEvent` ile mousePressed+mouseReleased gönderir (isTrusted=true).
2. `audio_injector.zig` `injectAnswerOnTarget()`:
   - PHASE 1 (Primary): JS ile submit butonunun bounding rect'i alınır (getBoundingClientRect, game-core iframe offseti eklenir), ardından `cdp.dispatchMouseClickAt(x, y)` ile isTrusted=true click yapılır.
   - PHASE 2 (Fallback): CDP click başarısız olursa eski JS submit yöntemleri kullanılır.
3. Bounds script doğru viewport koordinatlarını hesaplar: game-core iframe içindeki butonlar için `gc.getBoundingClientRect()` offseti eklenir; enforcement iframe'deki butonlar için direkt getBoundingClientRect kullanılır.
4. Tüm 545 test geçiyor, build temiz.

**Tekrar olmaması için:**
- Arkose/güvenlik kontrollü iframe'lerde submit daima CDP `Input.dispatchMouseEvent` ile yapılmalı, JS `element.click()` ASLA kullanılmamalı.
- Buton koordinatları hesaplanırken iframe zincirindeki her seviyenin viewport offseti hesaba katılmalı.
